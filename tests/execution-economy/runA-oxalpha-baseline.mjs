// runA-oxalpha-baseline.mjs — ox-alpha continuation baseline (Run A)
...
const PORT = 3080;
const LOG = path.join(os.tmpdir(), 'runA3-log.txt');
...
async function main() {
  ...
  const route = 'openrouter-continuation-test-' + Math.random().toString(16).slice(2, 10);
  const profile = { displayName: 'Cont Test A', apiKeyEnv: 'OPENROUTER_API_KEY', api: 'openai-completions',
  ...
  const ws = fs.mkdtempSync(path.join(os.tmpdir(), 'ee-cont-a-'));
  let sid = null;
  try {
