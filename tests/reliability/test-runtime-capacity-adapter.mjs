// test-runtime-capacity-adapter.mjs — Phase 02 R7 (R6-3) + adversarial: adapter
// wires the real (ASYNC) runtime resolveModelInfo into the capacity resolver;
...
async function main() {
  // T1: adapter wires ctx.get('runtime').resolveModelInfo (sync mock)
  {
    ...
    const res = await resolver.resolve('opencode', 'deepseek-v4-flash');
    check('T1 runtime capacity wins', res.window === 999000 && res.source === 'runtime', `window=${res.window} src=${res.source}`);
  }

  // T1b ADVERSARIAL: official resolveModelInfo is ASYNC — the adapter must await
  // the Promise and still return the window (previously read a Promise -> null).
  {
    const ctx = { get(k) { if (k === 'runtime') return { resolveModelInfo: async (p, m) => ({ context: { contextWindow: 888000 } }) }; return null; } };
    const wired = makeRuntimeCapacityResolver(ctx);
    check('T1b async runtime wired', wired.wired === true);
    const resolver = createCapacityResolver({ runtimeResolve: wired.runtimeResolve });
    const res = await resolver.resolve('opencode', 'deepseek-v4-flash');
    check('T1b async resolveModelInfo awaited -> window', res.window === 888000 && res.source === 'runtime', `window=${res.window} src=${res.source}`);
  }
  ...
  // T2: loose adapter finds runtime via ctx.runtime property
  {
    ...
    const resolver = createCapacityResolver({ runtimeResolve: wired.runtimeResolve });
    const res = await resolver.resolve('x', 'y');
    check('T2 loose runtime used', res.window === 777000, `window=${res.window}`);
  }
  ...
  // T3: no runtime available -> not wired -> resolver falls back to hints
  {
    ...
    const res = await resolver.resolve('commandcode', 'deepseek/deepseek-v4-flash');
  ...
  {
    const ctx = { get(k) { if (k === 'runtime') return { resolveModelInfo: async () => null }; return null; } };
    const wired = makeRuntimeCapacityResolver(ctx);
    const resolver = createCapacityResolver({ runtimeResolve: wired.runtimeResolve });
    const res = await resolver.resolve('commandcode', 'deepseek/deepseek-v4-flash');
  ...
  // T5: source-level check for the config injection point
  {
    ...
    check('T5 routers await capacity resolve', /await capacityResolver\.resolve/.test(fs.readFileSync(new URL('../../plugins/openrouter-router.mjs', import.meta.url), 'utf8')) && /await capacityResolver\.resolve/.test(src));
  }
