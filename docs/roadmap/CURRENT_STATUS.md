...
- **2026-09-24 06:2x：P3 官方 APPROVED backfill ＋ P4 生产激活授权回滚（External Reviewer 裁决执行）**：
  ...
  - **回滚执行（配置层，已实证）**：`~/.dsh/profiles/web/cordis.patch.yml` 移除 learn 注册块；
    ...
    **与 pre-P4 基线 `_backup-cordis.patch-learn-20260922-060153.yml` 逐字节一致**（含行尾）；
    YAML 有效（19 顶层条目）；`dsh --profile web --dump-config` 输出 19,982 B **不含 learn**；
  ...
  - **运行态（如实记录）**：当前进程 PID 18552 **仍加载 learn**（`RUNTIME_RELOAD_PENDING`）——
    dsh 无 canonical 插件热卸载；旧 `restart-dsh-server-delayed.ps1` 近 7 次尝试 5 FAILED + 1 卡 SPAWNED
    （全历史 108 次：COMMITTED 50 / FAILED 49 / SPAWNED 8 / STARTED 1），且 `restart-budget.json` 存在
    `candidateReady=true` 而 `stableCommitAt=null` 的悬挂候选 ⇒ **未通过「过去已真实 PASS」门槛，按 SAFETY 优先不强重启**。
    ⇒ `MANUAL_SAFE_RELOAD_REQUIRED`。**本回合唯一需要用户的操作。**
