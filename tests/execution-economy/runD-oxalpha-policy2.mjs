...
const PORT = 3080;
const LOG = path.join(os.tmpdir(), 'runD-log.txt');
...
async function main() {
  ...
  const route = 'openrouter-continuation-policy2-' + Math.random().toString(16).slice(2, 10);
  const profile = { displayName: 'Cont Test D', apiKeyEnv: 'OPENROUTER_API_KEY', api: 'openai-completions',
  ...
  const ws = fs.mkdtempSync(path.join(os.tmpdir(), 'ee-cont-d-'));
  let sid = null;
  try {
...
const baseTask = fs.readFileSync(path.join(os.tmpdir(), 'ee-cont-taskD.txt'), 'utf8');
const task = policy + '\n\n--- TASK ---\n\n' + baseTask;
