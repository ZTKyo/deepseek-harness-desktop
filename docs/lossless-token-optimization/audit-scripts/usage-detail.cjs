// 查看会话 usage 明细
// 用法: node usage-detail.cjs <sessionDir>
...
let totalInput = 0, totalCache = 0, totalOutput = 0, n = 0;
for (const l of lines) {
  ...
  if (e.type === 'assistant/message' && e.data?.usage) {
    ...
    totalInput += input; totalCache += cache; totalOutput += output; n++;
    console.log(`seq=${e.seq} turn=${e.data.turn} step=${e.data.step} input=${input} cacheRead=${cache} output=${output} model=${e.data?.message?.source?.model}`);
  }
}
console.log(`\ntotals: calls=${n} input=${totalInput} cacheRead=${totalCache} output=${totalOutput}`);
